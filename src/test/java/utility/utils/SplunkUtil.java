package utility.utils;

import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

import org.slf4j.Logger;
import org.testng.Assert;

import com.fasterxml.jackson.databind.DeserializationFeature;
import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import utility.pages.LoginPage;
import utility.pages.SearchPage;
import utility.request.SearchConfig;

public class SplunkUtil {
	private static final Logger logger = AppUtil.getLogger(SplunkUtil.class);
	private SearchPage searchPage;
	//		String jsonText = "{\"errormessage\":\"No message available\",\"severity\":\"critical\",\"requestmethod\":\"GET\",\"BackendArray\":{\"DCIResponseReason\":\"Not Found\",\"DCIResponseCode\":404,\"DCIMethod\":\"GET\",\"DCIResponse\":{\"path\":\"/apiService/v1/dci/dci\",\"error\":\"Not Found\",\"message\":\"No message available\",\"timestamp\":\"2020-09-25T12:55:49.856+0000\",\"status\":404},\"isTokenCached\":true,\"totalBackendResponseTime\":32,\"DCIRequestHeaders\":{\"authorization\":\"\",\"User-Agent\":\"Apache-HttpClient/4.5.3 (Java/1.8.0_261)\",\"X-Forwarded-Proto\":\"https\",\"X-API-KEY\":\"\",\"X-Forwarded-For\":\"27.62.211.246\",\"Host\":\"apigw.stag.staralliance.com\",\"cache-control\":\"no-cache\",\"X-Forwarded-Port\":\"443\"},\"DCIRequest\":{\"outboundFlightDate\":\"2019-10-18\",\"callingApplication\":\"LivingMapAA\",\"outboundFlightNumber\":\"AC839\",\"callingAirline\":\"1N\",\"inboundFlightNumber\":\"TK1587\",\"timeZone\":\"L\",\"inboundFlightDate\":\"2019-09-18\"},\"DCIResponseTimeInMilliSecs\":32},\"callingAirline\":\"1N\",\"requesterAirline\":\"1N\",\"sourcehost\":\"27.62.211.246\",\"requesturi\":\"/api/digitalConnectionInfo/v1/dci?callingAirline=1N&callingApplication=LivingMapAA&inboundFlightDate=2019-09-18&inboundFlightNumber=TK1587&outboundFlightDate=2019-10-18&outboundFlightNumber=AC839&timeZone=L\",\"source\":\"apigee\",\"transactionID\":\"1601038549866-rrt-07a91488af1df983e-b-de-10900-10759278-2\",\"statuscode\":404,\"environment\":\"test\",\"errordescription\":\"Not Found\",\"responseTimeInMilliSecs\":46,\"application\":\"digitalConnectionInfo-v1\",\"sourceip\":\"27.62.211.246\",\"loglevel\":\"error\",\"totalBackendResponseTime\":32,\"apirevision\":\"2\",\"severityID\":3,\"loggingAsyncTimeTaken\":1,\"requestpayload\":{\"outboundFlightDate\":\"2019-10-18\",\"callingApplication\":\"LivingMapAA\",\"outboundFlightNumber\":\"AC839\",\"callingAirline\":\"1N\",\"inboundFlightNumber\":\"TK1587\",\"timeZone\":\"L\",\"inboundFlightDate\":\"2019-09-18\"}}";

	public void loginToSplunk() {
		try {
			Assert.assertTrue(AppUtil.getDriver().getTitle().contains("Login"), "Not on login page");

			LoginPage loginPage = new LoginPage(AppUtil.getDriver());
			loginPage.enterUsername(AppUtil.getProperty("username"));
			loginPage.enterPassword(AppUtil.getProperty("password"));
			loginPage.clickLogin();
		} catch (Exception e) {
			logger.error("Login error :" + e.getMessage());
			e.printStackTrace();
		}
	}

	public List<JsonNode> getSearchResult(SearchConfig config, int numberOfRecords) {
		List<JsonNode> searchResults = new ArrayList<JsonNode>();
		searchPage = new SearchPage();

		if (config.getPresets() != null)
			searchPage.setTimeRangePresets(config.getPresets().getName());
		// search query
		searchPage.searchSplunkQuery(config.getSearchQuery());

		// capture result
		for (int index = 1; index <= numberOfRecords; index++) {
			String jsonText = searchPage.getResultAsRawText(index);
			searchResults.add(getJsonNode(jsonText));
		}

		return searchResults;
	}

	public void assertNodePresent(JsonNode rootNode, String path) {
		Assert.assertFalse(rootNode.at(path).isMissingNode(), "Path " + path + "is not present at node");
	}

	public void assertValue(JsonNode rootNode, String path, String expectedValue) {
		assertNodePresent(rootNode, path);
		Assert.assertEquals(rootNode.at(path).asText(), expectedValue,
				expectedValue + "is not present at " + path + "in root node");
	}

	public void assertValue(JsonNode rootNode, String path, int expectedValue) {
		assertNodePresent(rootNode, path);
		Assert.assertEquals(rootNode.at(path).asInt(), expectedValue,
				expectedValue + "is not present at " + path + "in root node");
	}

	public void assertValue(JsonNode rootNode, String path, double expectedValue) {
		assertNodePresent(rootNode, path);
		Assert.assertEquals(rootNode.at(path).asDouble(), expectedValue,
				expectedValue + "is not present at " + path + "in root node");
	}

	public void assertValue(JsonNode rootNode, String path, boolean expectedValue) {
		assertNodePresent(rootNode, path);
		Assert.assertEquals(rootNode.at(path).asBoolean(), expectedValue,
				expectedValue + "is not present at " + path + "in root node");
	}

	public void assertValue(JsonNode rootNode, String path, long expectedValue) {
		assertNodePresent(rootNode, path);
		Assert.assertEquals(rootNode.at(path).asLong(), expectedValue,
				expectedValue + "is not present at " + path + "in root node");
	}

	/**
	 * @param rootNode       - input node
	 * @param pathOfExpected - path of expected node
	 * @param pathOfActual   - path of actual node (node under test)
	 */
	public void compareNode(JsonNode rootNode, String pathOfExpected, String pathOfActual) {
		assertNodePresent(rootNode, pathOfExpected);
		assertNodePresent(rootNode, pathOfActual);
		Assert.assertEquals(rootNode.at(pathOfExpected), rootNode.at(pathOfActual),
				"Json node expected at path : " + pathOfExpected + " is not present at " +
						pathOfActual + "in root node.\n");
	}

	private JsonNode getJsonNode(String value) {
		JsonNode result = null;
		try {
			result = getMapper().readTree(value);
		} catch (IOException e) {
			System.out.println("Error occurred while reading json : ");
			e.printStackTrace();
		}

		return result;
	}

	private ObjectMapper getMapper() {
		ObjectMapper mapper = new ObjectMapper();
		mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
		return mapper;
	}

}
